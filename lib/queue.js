...
const add = async (redis, queueName, jobData, jobResult) => {
  ...
  jobResult.links
    .filter((link) => !snapshot.queue.queued.includes(link) && !snapshot.queue.processing.includes(link) && !snapshot.queue.done.includes(link))
  ...
  await pipeline.exec();
  return await getSnapshot(redis, queueName);
...
const stop = (redis, queueName) => async () => {

  return await redis.hset(`sc:${queueName}:state`, 'status', 'INACTIVE');
