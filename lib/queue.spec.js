...
const { logger } = require('../test/fixture');
const {
  register,
  deregister,
  add,
...
const { QueueEndError } = require('./error');
...
describe('queue @queue', () => {
  ...
  describe('register', () => {
    ...
    context('when it is the first run', () => {
      ...
      it('creates a new job in redis', async () => {
        ...
        const {
          snapshot, config,
        } = await register(redis, createJob, logger);
    ...
    context('when it is not the first run', () => {
      ...
      it('pops a job from redis', async () => {
        ...
        const { snapshot } = await register(redis, job, logger);
    ...
    context('when the depth exceeds the limit', () => {
      ...
      it('throws an error', async () => {
        ...
        await rejects(async () => register(redis, job, logger), /has reached depth limit/);
    ...
    context('when queue status is not active', () => {
      ...
      it('throws an error', async () => {
        ...
        await rejects(async () => register(redis, job, logger), /is not active/);
    ...
    context('when the number of active jobs is at the limit', () => {
      ...
      it('throws an error', async () => {
        ...
        await rejects(async () => register(redis, job, logger), /has reached concurrency limit/);
    ...
    context('when the number of active jobs is under the limit', () => {
      ...
      it('adds the current job to the \'processing\' set', async () => {
        ...
        await register(redis, job, logger);
    ...
    context('when the job exists in the \'processing\' set', () => {
      ...
      it('throws an error', async () => {
        ...
        await rejects(async () => register(redis, jobExisting, logger), /has already processed/);
    ...
    context('when there are no more jobs in the queue', () => {
      ...
      it('throws an error', async () => {
        ...
        await rejects(async () => register(redis, job, logger), /has no more queued jobs/);
  ...
  describe('deregister', () => {
    ...
    it('removes the job from the \'processing\' set and adds the job to the \'done\' set', async () => {
      ...
      await deregister(redis, 'automated-test-queue', jobData, logger);
    ...
    context('when deregistering a job that does not exist in the processing queue', () => {
      ...
      it('throws an error', async () => {
        ...
        await rejects(() => deregister(redis, 'automated-test-queue', jobData, logger), Error);
  ...
  describe('add', () => {
    ...
    context('when the job already exists in processing, done, or queued', () => {
      ...
      it('does not add the job to the queue', async () => {
        ...
        await add(redis, 'automated-test-queue', jobData, jobResult, logger);
  ...
  describe('stop', () => {
    ...
    it('aborts the current job and signals the queue to end', async () => {
      ...
      await rejects(async () => stop({ url: 'https://locust.dev' }, logger)(), QueueEndError);
