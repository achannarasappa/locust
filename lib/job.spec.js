...
describe('job', () => {
  ...
  describe('runJob @server', () => {
    ...
    it('responds with the result of the scrape', async () => {
      ...
      const output = await runJob(browser, {}, jobData, snapshot, logger);
    ...
    describe('user defined lifecycle hooks', () => {
      ...
      context('when the "beforeStart" hook is defined', () => {
        ...
        it('runs the hook', async () => {
          ...
          await runJob(browser, jobDefinition, jobData, snapshot, logger);
      ...
      context('when the "extract" hook is defined', () => {
        ...
        it('runs the hook', async () => {
          ...
          const { data: output } = await runJob(browser, jobDefinition, jobData, snapshot, logger);
        ...
        it('calls the hook with a selection function, page object, browser object, and jobData', async () => {

          const extractHook = sinon.fake();
          const jobDefinition = { extract: extractHook };
          const jobData = { url: 'http://localhost:3001/page-fixture' };
          const snapshot = { state: { firstRun: 1 } };

          await runJob(browser, jobDefinition, jobData, snapshot, logger);

          assert.ok(extractHook.calledOnce);
          assert.ok(
            extractHook.calledWithMatch(
              sinon.match.func,
              sinon.match.instanceOf(Page),
              sinon.match.instanceOf(Browser),
              jobData,
            ),
          );
        ...
        context('when there is an error selecting an element on the page', () => {
          ...
          it('returns null', async () => {
            ...
            const { data: output } = await runJob(
              browser,
              jobDefinition,
              jobData,
              snapshot,
      ...
      context('when the browser encounters an error', () => {
        ...
        it('it throws an error', async () => {
          ...
          await rejects(() => runJob(browser, {}, jobData, snapshot, logger), BrowserError);
